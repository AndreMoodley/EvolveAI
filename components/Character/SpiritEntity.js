...
const CH = 268;
const CX = CW / 2; // 140
const CY = 148;    // vertical orbit/aura center
...
// Outer / middle / inner aura ring diameters per realm
const RING_D = {
  1: [128,  92,  62],
  2: [144, 104,  70],
  3: [158, 114,  78],
...
// Particle configs [radius, duration, dot-size]
const PCFG = [
...
// Realm opacity ramp: dim at realm 1, fully solid at realm 7
...
function Particle({ index, total, radius, dur, dot, color }) {
  ...
  return (
    ...
    >
      <View style={{
        position: 'absolute',
        top: 0, left: radius - dot / 2,
        width: dot, height: dot, borderRadius: dot / 2,
        backgroundColor: color,
        shadowColor: color, shadowOpacity: 0.9, shadowRadius: dot * 2, elevation: 4,
      }} />
    </Animated.View>
  );
}
...
export default function SpiritEntity({
  ...
  const auraColor = AURA_COLORS[auraKey]?.color ?? AURA_COLORS.ki.color;
  ...
  // Eye color: tracks ki level
  const eyeColor = ki > 60 ? auraColor : ki > 30 ? '#f5b301' : '#ff5b76';

  // Arm spread driven by shadowLevel
  const armDeg = 22 + (shadowLevel - 1) * 7; // 22 → 50 degrees
  ...
  useEffect(() => {
    ...
    loop(eyeAnim, 850).start();
  }, []);

  useEffect(() => {
    Animated.spring(armAnim, {
      toValue: armDeg, useNativeDriver: true, friction: 7, tension: 30,
    }).start();
  }, [shadowLevel]);

  // --- Layout constants ---
  const HEAD_W = 44, HEAD_H = 44;
  const HEAD_TOP = 46, HEAD_LEFT = CX - HEAD_W / 2;       // center=(140, 68)

  const NECK_W = 12, NECK_H = 14;
  const NECK_TOP = HEAD_TOP + HEAD_H + 1;                  // 91
  const NECK_LEFT = CX - NECK_W / 2;                      // 134

  const TORSO_W = 50;
  const TORSO_TOP = NECK_TOP + NECK_H;                     // 105
  const TORSO_LEFT = CX - TORSO_W / 2;                    // 115

  const ARM_W = 11;
  const ARM_TOP = TORSO_TOP + 6;                           // 111
  const ARM_L_LEFT = TORSO_LEFT - ARM_W - 4;              // 100
  const ARM_R_LEFT = TORSO_LEFT + TORSO_W + 4;            // 169

  const LEG_W = 18, LEG_H = 52;
  const LEG_TOP = TORSO_TOP + stage.torsoH + 2;
  const LEG_L_LEFT = CX - LEG_W - 3;                      // 119
  const LEG_R_LEFT = CX + 3;                              // 143
  ...
  return (
    <View style={{ width: CW, height: CH, alignSelf: 'center' }}>
      ...
      <Animated.View
        pointerEvents="none"
        style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, opacity: charOpacity, transform: [{ scale: bodyScale }] }}
      >
        {/* Head */}
